...
def fetch_product_description(link):
    try:
        ...
        # Find SKU Code
        sku_tag = product_soup.find('span', class_='c126 c128')
        if sku_tag and 'SKU Code' in sku_tag.text:
            sku_code = sku_tag.text.split(':')[-1].strip()
        else:
            sku_code = "SKU Code not found"
    ...
    if not product_cards:
        ...
        for product in product_cards:
            ...
            if name_tag and price_tag and link_tag:
                ...
                sku_code, specifications = fetch_product_description(full_link)
